...
export function run() {
    for (var i = 0; i < world.getPlayers().length; i++) {
        ...
        if (scoreboard.getObjective("play") != null && scoreboard.getObjective("play").getScore(player[i]) == 1) {
            //on wall
            ...
            if (scoreboard.getObjective("fence").getScore(player[i]) == 1 && player[i].isFalling == false && player[i].isJumping) {
                ...
                player[i].runCommandAsync("execute anchored feet as @s at @s rotated ~ 0 run tp @s ^^^0.5")
